...
import { addAnnouncementComment, deleteAnnouncementComment } from '@/lib/data';
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const comment = await addAnnouncementComment(
      id,
      {
        id: user.id,
    ...
    console.error('Add announcement comment error:', error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await deleteAnnouncementComment(commentId, user.id, user.role);
